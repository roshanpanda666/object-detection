...
const ObjectDetection = () => {
  ...
  const [isAnnouncementActive, setIsAnnouncementActive] = useState(false); // Tracks if announcement is currently active
  ...
  const announcePersonDetected = () => {
    setIsAnnouncementActive(true); // Activate announcement
    const msg = new SpeechSynthesisUtterance("Person detected");
  ...
  useEffect(() => {
    ...
    return () => {
      clearInterval(detectInterval); // Cleanup the interval
    };
  }, []);
